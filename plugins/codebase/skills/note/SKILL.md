...
Each note records what was true when written, so confirm the named file, command, or version still holds before relying on it.

Below is this repo's INDEX.md. Read any note that fits the task.

!`cat ${CLAUDE_PLUGIN_DATA}/notes/$ARGUMENTS/INDEX.md 2>/dev/null || echo '<empty>'`
